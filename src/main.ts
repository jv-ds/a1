...
/**
 * Tests overlap between two axis-aligned rectangles.
 * @param a first Rect
 * @param b second Rect
...
const tick = (s: State) => {
  ...
  const pendingAfter = s.pending.filter(
    it => it.appearTime > elapsedMsNext,
  ); //filters schedule to keep ONLY pipes that appear after current tick
  ...
  const inFramePipes = s.pipes.concat(newPipes).filter(p => p.frame + Constants.PIPE_WIDTH >= s.scrollX);
  ...
  const newlyPassed = inFramePipes
    ...
    .filter(rightEdge => rightEdge > birdFrameXPrev && rightEdge <= birdFrameXNext).length;
  ...
  const scoreAfter = !anyHit
    ? s.score + newlyPassed
    : s.score; //if hit, don't award point this tick (avoids doubling up), if no hit, increase by newlypassed
  ...
  //takes away life
  const livesAfter = anyHit && canTakeHit ? Math.max(0, s.birdLives - 1) : s.birdLives;

  //if hit happened, reset cooldown to 6 ticks, else reduce by 1
  const hitCooldownAfter = anyHit && canTakeHit ? 6 : Math.max(0, s.hitCooldown - 1);
...
const render = (): ((s: State) => void) => {
  ...
  return (s: State) => {
    frame.setAttribute(
      "transform",
      `translate(${-s.scrollX}, 0)`,
    ); //transform updates all children at once, shifts co-ordinates to move to the right (neg moves right)
